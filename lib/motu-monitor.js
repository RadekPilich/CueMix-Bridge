...
const { noiseGrp, COLORS, TYPE_COLORS, TYPE_MARGIN, throttleTime } = require('../config/config-monitor');
...
const logger = (type, message) => {
	...
	// REDUCE HIGH FREQUENCY MESSAGES
		...
		// Setup types (CONTROL, INPUT, SYSTEM, ERROR) should ALWAYS show up.
			...
			if (highFrequencyTypes.includes(upperType)) {
				// Extract the first word containing letters to ignore leading symbols like "> "
				const match = `${message}`.match(/[a-z]+/i);
				const messagePart = match ? match[0].toUpperCase() : 'DEFAULT';
				const throttleKey = `${upperType}_${messagePart}`;
				const lastTime = lastLogTimes.get(throttleKey) || 0;
				
				if (now - lastTime < throttleTime && now - lastTime > 5 )  {
				//	console.log(`${COLORS.dim}${throttleKey} message throttled: ${message}${COLORS.reset}`);
					return; 
				}
	...
	// TEXT CENTERING
		...
		//	const centeredText = upperType.padStart(leftPad + upperType.length).padEnd(12);
			...
			const timestamp = new Date().toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', second: '2-digit', fractionalSecondDigits: 3 });
