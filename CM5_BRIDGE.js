...
// --- VALUE CONVERSIONS --- ------------------------------------------------------------------
	...
	const flipCC = (val) => 127 - val; // 0-127 reversal

	const flipCCtoTrim = (val) => {
		const result = 100 - (val * 100 / 127);
		return Math.max(0, Math.min(100, Math.round(result)));
	};

	const flipTrimToCC = (val) => {
		const result = 127 - (val * 127 / 100);
		return Math.max(0, Math.min(127, Math.round(result)));
	};

	const flipPitchToTrim = (val) => {
		const result = 100 - (val * 100 / 16384);
		return Math.max(0, Math.min(100, Math.round(result)));
	};

	const flipPitchToCC = (val) => {
		const result = 127 - (val * 127 / 16384);
		return Math.max(0, Math.min(127, Math.round(result)));
	};
	...
	const ccToFader = (val) => {
		...
		logger('invalues', `ccToFader: ${buf.readUInt32BE(0)}`);
		return buf;
	};

	const pitchToFader = (val) => {
		let scaled;
		if (val < 12800) {
		...
		const buf = Buffer.alloc(4);
		buf.writeUInt32BE(scaled);
		logger('invalues', `pitchToFader: ${buf.readUInt32BE(0)}`);
		return buf;
	};

	const pitchToTrim = (val) => {
		let scaled;
...
ws.on('message', (data) => {
    ...
    if (cm5map.faders[cmdId] && data.length == 8) {
            ...
            midiVal = Math.min(Math.max(midiVal, 0), 127);
...
const handleControlChange = (msg) => {
    ...
    if (faderMatches.length > 0) {
        ...
        faderMatches.forEach(([idHex]) => {
            const valBuf = ccToFader(msg.value); 
...
midiIn2.on('pitch', (msg) => {
	...
	if (pitchTrimMatches.length > 0) {
		...
		const valBuf = pitchToTrim(msg.value);
    ...
    if (pitchFaderMatches.length > 0) {
		logger('input',`> PITCH > 0CHAN: ${msg.channel} VAL: ${msg.value}`);
        lastSentValue[getPitchId(msg)] = msg.value; 
        const valBuf = pitchToFader(msg.value);
